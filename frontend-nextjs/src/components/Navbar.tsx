...
// Note: Ensure you have your logo in /public/assets/MetaCowLogo.png 
// or update the src below. For now I'll use a placeholder if missing.
...
const LiquidityIcon = (props: any) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M12 2c-5.33 4.55-8 8.48-8 11.91 0 3.22 2.69 5.82 6 5.82 2.24 0 4.25-1.18 5.31-2.95.14-.23.49-.23.62 0C16.75 18.64 18.76 19.73 21 19.73c3.31 0 6-2.6 6-5.82C27 10.48 23.33 6.55 18 2L12 2zm0 15.91c-1.48 0-2.75-1.04-2.95-2.43H12v-1.79h-2.95c-.2-.79-1.04-1.63-2.95-1.63s-2.75 1.04-2.95 2.43H3.1c.2-.79 1.04-1.63 2.95-1.63s2.75 1.04 2.95 2.43H12v-1.79h-2.95c-.2-.79-1.04-1.63-2.95-1.63s-2.75 1.04-2.95 2.43H3.1c.2-.79 1.04-1.63 2.95-1.63s2.75 1.04 2.95 2.43H12v-1.79h2.95c.2.79 1.04 1.63 2.95 1.63s2.75-1.04 2.95-2.43h.05c-.2.79-1.04 1.63-2.95 1.63s-2.75-1.04-2.95-2.43H12v1.79h2.95c.2.79 1.04 1.63 2.95 1.63s2.75-1.04 2.95-2.43h.05c-.2.79-1.04 1.63-2.95 1.63s-2.75-1.04-2.95-2.43H12v1.79z" /></svg>
);
...
export default function Navbar() {
  ...
  useEffect(() => {
    // Only check network if connected and chainId is populated
            ...
            <div className="flex items-center gap-4">
              {isConnected && address ? (
                <div className="relative">
                  ...
                  >
                    <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-2xl shadow-2xl p-4 z-50 origin-top-right">
                        <div className="flex items-center gap-4 pb-4 border-b border-gray-100">
                            <div className="w-10 h-10 rounded-full bg-gray-100 border-2 border-white shadow-sm flex items-center justify-center text-2xl">
                                🐮
                            </div>
                            <div>
                        ...
                        <div className="py-4 space-y-3 text-sm">
                            ...
                            <div className="flex justify-between items-center"><span className="text-gray-500">Balance</span><span className="font-semibold text-gray-800">{parseFloat(balance).toFixed(4)} tBNB</span></div>
                        </div>
                        <button onClick={() => { disconnect(); setShowDropdown(false); }} className="w-full text-center bg-red-50 text-red-600 py-2.5 rounded-lg hover:bg-red-100 transition-all duration-200 font-semibold text-sm">
                            Disconnect
                        </button>
                    </div>
                  </Transition>
